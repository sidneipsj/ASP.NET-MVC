...
namespace DAL
{
    public class DALEmpresa
    {
        ...
        public static DataSet FormasPgLiberadasParcelamentoEspecifico(int empres_id, int cred_id)
        {
            if (CredenciadoAceitaParcelamento(cred_id) == true)
            {
                String sql = "SELECT A.FORMA_ID, B.DESCRICAO FROM FORMAS_EMP_LIB AS A, FORMASPAGTO AS B WHERE "
                                      + "A.EMP_ID = " + empres_id + " AND A.FORMA_ID = B.FORMA_ID AND B.LIBERADO = 'S' ORDER BY A.FORMA_ID";
            ...
            {
                String sql = "SELECT A.forma_id, C.descricao FROM FORMAS_CRED_LIB A " +
                    "INNER JOIN FORMAS_EMP_LIB B ON A.forma_id = B.forma_id " +
                    "AND A.CRED_ID = " + cred_id + " AND B.EMP_ID = " + empres_id +
                    "INNER JOIN FORMASPAGTO C ON C.FORMA_ID = A.FORMA_ID";
        ...
        public static bool GetUsaParcelamentoEspecifico(int empres_id)
        {
            BD bd = new BD();
            String usaParcelamentoEspecifico = bd.ExecuteScalar("", "select usa_parcelamento_especifico from empresas WHERE empres_id = " + empres_id, null);
            if (usaParcelamentoEspecifico.Equals("S", StringComparison.InvariantCultureIgnoreCase))
            {
                return true;
            }
            else return false;
