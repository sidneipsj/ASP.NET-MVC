...
namespace DAL
{
    public class DALConveniado
    {
        ...
        /// <summary>
        /// BUSCA AS AUTORIZACOES PARA EXIBIR NO RELATORIO REPASSE
        /// </summary>
        /// <param name="credID">Cód. do Estabelecimento</param>
        /// <param name="dtIni">Data Inicial</param>
        /// <param name="dtFim">Data Final</param>
        /// <returns>DataTable com a busca</returns>
        public static DataTable GetAutorizacoesRelRepasse(int credID, string dtIni, string dtFim)
        {
            string sql;

            SqlParamsList ps = new SqlParamsList();
            ps.Add(new Fields("@credID", credID));
            ps.Add(new Fields("@dtIni", dtIni));
            ps.Add(new Fields("@dtFim", dtFim));

            sql = "SELECT DISTINCT conv.chapa,"
                + "conv.titular,"
                + "emp.empres_id,"
                + "emp.FANTASIA,"
                + "cc.debito,"
                ...
                + "cc.data, cc.hora, "
                + "cc.nf,"
                + "(concat(CAST(cc.autorizacao_id AS varchar(10)),"
                + "        (CASE"
                + "          WHEN cc.digito < 10 THEN"
                + "           '0' + CAST(cc.digito AS char(1))"
                + "          ELSE"
                + "           CAST(cc.digito AS char(2))"
                + "        end))) AS autorizacao,"
                + "cast(debito - credito as decimal(15, 2)) saldo,"
                + "CONCAT(CASE WHEN aut.FORMAPAGTO_ID = 1 THEN 1 "
                + "ELSE substring(aut.HISTORICO,10,2) END,'/',aut.FORMAPAGTO_ID) AS PARCELAS"
                + "  FROM Contacorrente cc"
                + "  JOIN Conveniados conv ON conv.conv_id = cc.conv_id"
                + "  JOIN Empresas emp ON conv.empres_id = emp.empres_id"
                + "  join Autor_Transacoes aut ON cc.AUTORIZACAO_ID = aut.AUTOR_ID"
                + " WHERE cc.cred_id = @credID"
                + "   AND (cc.autorizacao_id_canc is null and cc.cancelada = 'N')"
                + "   AND cc.data BETWEEN @dtIni AND @dtFim "
                + "AND cc.EMPRES_ID not in(SELECT EMPRES_ID FROM empresas WHERE TEM_VALOR_FINANCEIRO = 'N')"
                + " ORDER BY cc.data, cc.hora,titular";
        ...
        public static Conveniados IncluiConv(Conveniados pConv, string pOperador, int cred_id, string cartao_ativo, string codigoImportacao, Bandeiras_Conv band_conv, int nProtocolo)
        {
            ...
            MontadorSql mont = new MontadorSql("conveniados_bem", MontadorType.Insert);
            ...
            if (incluiu == 1)  //se entrar cria o cartão
            {
                ...
                Cartoes cart = new DALCartao().IncluiCartao(pConv.Conv_id, Utils.TirarAcento(pConv.Titular), pConv.LimiteMes, pConv.Status, pOperador, "S", 0, cred_id, "S", pConv.DtNasc, codigoImportacao, nProtocolo);
        ...
        //Alterado para SqlServer
        public static int GeraIdHistoricoConveniados()
        {
            BD BD = new BD();
            return Convert.ToInt32(BD.ExecuteScalar(" SELECT NEXT VALUE FOR SID_HISTORICO_CONV AS ID ", null).ToString());
        }
        ...
        public static DataRow GetConv(string cartao_id)
        {
            ...
            sql += " cart.codigo, cart.digito, cart.cartao_id, conv.fidelidade AS convfidelidade, emp.fidelidade AS empfidelidade, emp.prog_desc, emp.band_id, emp.liberada as emp_lib \n";
            sql += " FROM Conveniados conv \n";
            sql += " JOIN Cartoes cart ON cart.conv_id = conv.conv_id \n";
            sql += " JOIN Empresas emp ON conv.empres_id = emp.empres_id \n";
            sql += " JOIN bandeiras b ON b.band_id = emp.band_id \n";
        ...
        public static string getSenhaConvNaTabelaConveniados(string num_cartao)
        {
            SqlParamsList ps = new SqlParamsList();
            ps.Add(new Fields("num_cartao", num_cartao));

            string sql = " SELECT conv.senha ";
            sql += " FROM CONVENIADOS conv INNER JOIN CARTOES cart ON conv.conv_id = CART.conv_id and cart.codigo = @num_cartao and cart.apagado <> 'S'";
        ...
        public static void GravaHistoricoAtualizouLimiteMes(int conv_id,int empres_id, float valorLimiteNovo, float valorLimiteAnterior)
        {
            int inclui,id;
            DateTime DataHoraAtual = new DateTime();
            DateTime Data = new DateTime();
            DataHoraAtual = DateTime.Now;
            Data = DataHoraAtual.Date;
            id = GeraIdHistoricoConveniados();

            BD BD = new BD();
            string sql = "";
            sql = string.Format("insert into HISTORICO_CONVENIADO (ID,DESCRICAO,CONV_ID,EMPRES_ID,VALOR_ANTERIOR,VALOR_ATUAL,DATA_HORA,DATA) values ({0}, 'Atualizou limite mês','{5}','{6}', '{1}', '{2}', '{3}', '{4}' )", id, Convert.ToString(valorLimiteAnterior), Convert.ToString(valorLimiteNovo), DataHoraAtual, Data,conv_id,empres_id);

            inclui = BD.ExecuteNoQuery(sql,null);
        }

        public static bool ValidaAlteracaoDeLimiteMes(int conv_id)
        {
            BD BD = new BD();
            string sql = "select DATEDIFF(MM,data,GETDATE()) periodo "+ 
	                     "from historico_conveniado "+ 
	                     "where "+
                         "DATEDIFF(MM,data,GETDATE()) < 1 and conv_id = "+conv_id;
            int intervaloUltimaAlteracao = (int)BD.ExecuteScalar(sql,null);
            if (intervaloUltimaAlteracao > 1)
                return true;
            else
                return false;
