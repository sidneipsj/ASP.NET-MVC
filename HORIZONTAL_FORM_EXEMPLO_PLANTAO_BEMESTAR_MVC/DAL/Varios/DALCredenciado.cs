    ...
    public class DALCredenciado
    {
        ...
        public static DataTable RelatorioVendasCantinex(int cred_id, string data1, string data2)
        {
            ...
            sql = " SELECT CONVERT(VARCHAR(10),CC.DATAVENDA,103) AS DATA,  CART.NOME,(concat(CAST(cc.autorizacao_id AS varchar(10)),"
                ...
                + " WHERE CC.DATAVENDA BETWEEN @dtInicial AND @dtFinal and CC.CRED_ID = @credID"
        ...
        public static DataTable DataDosPagamentosBaixados(int cred_id)
        {
            string sql;
            sql = "SELECT DISTINCT DATA_COMPENSACAO as data_pgto,month(data_pgto) mes " +
                  "FROM PAGAMENTO_CRED WHERE CRED_ID = " + cred_id + " AND DATA_PGTO >= '01/03/2015' order by data_pgto DESC";
            BD BD = new BD();
            return BD.GetDataTable(sql, null);
        } 

        public static DataTable DadosRepasseBaixados(string pagCred)
        {
            string sql;
            sql = "SELECT A.PAGAMENTO_CRED_ID, " +
                  "(SELECT DISTINCT CONVERT(varchar,A.DATA_COMPENSACAO,103)) as LIQUIDADO, " +
                  "(SELECT DISTINCT CONVERT(varchar,A.DATA_PGTO,103)) as VENCIMENTO, " +
                  "A.CRED_ID, A.PAGA_CRED_POR_ID" +
                  " FROM PAGAMENTO_CRED A WHERE A.PAGAMENTO_CRED_ID = " + pagCred;
        ...
        {
            string sql;
            sql = "SELECT CASE WHEN TR.TAXA_ID <= 0 THEN TR.HISTORICO " +
                          "ELSE T.DESCRICAO END DESCRICAO," +
                          "CASE WHEN TR.TAXA_ID <= 0 THEN TR.VALOR " +
                          "ELSE T.VALOR END VALOR " +
                          "FROM TAXAS_REPASSE TR " +
                          "LEFT JOIN TAXAS T ON TR.TAXA_ID = T.TAXA_ID " +
