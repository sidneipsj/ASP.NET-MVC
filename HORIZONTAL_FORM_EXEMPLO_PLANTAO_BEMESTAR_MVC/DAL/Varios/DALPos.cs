...
namespace DAL
{
  public class DALPos
  {
    ...
    public static string GetVersaoOS(string numSerie)
    {
        SqlParamsList ps = new SqlParamsList();
        ps.Add(new Fields("@serial_number", numSerie));
        BD BD = new BD();
        string s = Convert.ToString(BD.ExecuteScalar("", "select p.versao_os from pos p where p.pos_serial_number = @serial_number", ps));
        return s;
    }

    public static string GetVersaoEOS(string numSerie)
    {
        SqlParamsList ps = new SqlParamsList();
        ps.Add(new Fields("@serial_number", numSerie));
        BD BD = new BD();
        string s = Convert.ToString(BD.ExecuteScalar("", "select p.versao_eos from pos p where p.pos_serial_number = @serial_number", ps));
        return s;
    }
